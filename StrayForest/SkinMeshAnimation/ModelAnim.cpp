...
void CSkinMesh::RenderMeshContainer(LPDIRECT3DDEVICE9 pDevice, MYMESHCONTAINER* pMeshContainer, MYFRAME* pFrame)
{
	...
	UINT numpass;
	EffectShaderManager::GetEffect(SKINMESH)->Begin(&numpass, D3DXFX_DONOTSAVESHADERSTATE);
	//スキンメッシュの描画
	if (pMeshContainer->pSkinInfo != NULL)
	{
...
void CSkinMesh::ChangeAnim(DWORD _NewAnimNum) {
	...
	m_pAnimController->GetTrackDesc(m_CurrentTrack, &m_CurrentTrackDesc);
	m_CurrentTrackDesc.Position = 0;
	m_pAnimController->SetTrackDesc(m_CurrentTrack, &m_CurrentTrackDesc);
